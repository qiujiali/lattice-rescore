...
def convert(trn=None, ctm=None):
    ...
    for line in content:
        ...
        col1 = (segm_info[0] + "_" + segm_info[1] + "_" + segm_info[2]
                + "_" + segm_info[3])
        col2 = segm_info[4]
        start_time_int = int(segm_info[9])
        end_time_int = int(segm_info[10])
        diff_int = end_time_int - start_time_int
        word_split = split[0].split(" ")
        word_split = list(
            filter(lambda x: len(x) > 0 and any([c != " " for c in x]),
            word_split))
        if len(word_split) > 0:
...
def get_parser():
    ...
    parser.add_argument("trn", type=str, default=None, nargs="?",
                        help="input trn")
    parser.add_argument("ctm", type=str, default=None, nargs="?",
                        help="output ctm")
    return parser
