...
def convert(trn=None, stm=None, orig_stm=None):
    if orig_stm is not None:
        with codecs.open(orig_stm, 'r', encoding="utf-8") as orig_stm:
            ...
            for line in orig_content:
                (header if line.startswith(";;") else content).append(
                    line.strip())
            del orig_content
    ...
    for i, line in enumerate(content):
        ...
        col1 = (segm_info[0] + "_" + segm_info[1] + "_" + segm_info[2]
                + "_" + segm_info[3])
        col2 = segm_info[4]
        col3 = (segm_info[5] + "_" + segm_info[6] + "_" + segm_info[7]
                + "_" + segm_info[8])
        start_time = str(int(segm_info[9]))
        end_time = str(int(segm_info[10]))
        col4 = (start_time[:-2] if len(start_time) > 2 else "0") + "." + (
            start_time[-2:] if len(start_time) > 1 else "00")
        col5 = ((end_time[:-2] if len(end_time) > 2 else "0")
...
def get_parser():
    ...
    parser.add_argument('--orig-stm', type=str, default=None, nargs='?',
                        help="Original stm file to add additional information "
                             "to the generated one")
    parser.add_argument('trn', type=str, default=None, nargs='?',
                        help='input trn')
    parser.add_argument('stm', type=str, default=None, nargs='?',
                        help='output stm')
    return parser
