...
# Copyright 2015  Cambridge Univeristy (Yanmin Qian): converting to the FSM which is used in HTK lattices convertion tools
...
$usage="Convert kaldi lattices to HTK SLF (v1.1) format.\n".
       ...
       "  --original-scores  Leave language and acoustic scores as they are. (default: merge language scores into acoustic scores)".
       "  --tscale x  Transition probability scaling factor. (In kaldi transp stored in the LM score, and in HTK it is needed to be merged into AM score) (default: $tscale)\n".
...
while (@ARGV gt 0 and $ARGV[0] =~ m/^--/) {
  ...
  elsif ($param eq "--tscale") { $tscale = shift @ARGV;}
  elsif ($param eq "--word-to-node") { $wordtonode = 1;}
  elsif ($param eq "--original-scores") { $originalscores = 1;}
  else {
...
while(<FI>) {
  ...
  if (@A == 1 and $utt eq "") {
    ...
    $accepting_states{$A[0]} = { W=>"</s>", v=>0, a=>0, l=>0 };
    ...
    # add the link data
    $accepting_states{$A[0]} = { W=>"</s>", v=>0, a=>$as, l=>$gs };
   ...
   # $w = "<eps>" if $w eq "<eps>";
    ...
    $nodes{$last_node} = { W=>"</s>", t=>$latest_time };
    ...
    for $sink (sort { $a <=> $b } keys %sinks) {
      unless(exists($accepting_states{$sink})) {
        ...
        $a = \$accepting_states{$accept};
        push @links, { S=>$accept, E=>$last_node, W=>"</s>", v=>0, a=>0, l=>0 };
      }
    }
 ...
 #       printf FH "I=%d\tt=%.2f\n", $n, $nodes{$n}{t}*$framerate;
      ...
      printf FH "0\t1\t<s>\t<s>\t0.00\t0.000000\t0.000000\n";
      ...
      for $i (0 .. $#links) {
        ...
        if ($originalscores == 0) {
          printf FH "%d\t%d\t%s\t%s\t%.2f\t%f\t0.000000\n", $l{S}+1, $l{E}+1, $l{W}, $l{W}, $nodes{$l{E}}{t}*$framerate, $l{a}+$l{l}*$tscale;
        }
        else {
          printf FH "%d\t%d\t%s\t%s\t%.2f\t%f\t%f\n", $l{S}+1, $l{E}+1, $l{W}, $l{W}, $nodes{$l{E}}{t}*$framerate, $l{a}, $l{l};
        }
        $end_node=$l{E}+1;
      }
      ...
      # nodes
      for $n (sort { $a <=> $b } keys %nodes) {
        printf FH "I=%d\tt=%.2f\tW=%s\n", $n, $nodes{$n}{t}*$framerate, $nodes{$n}{W};
      }
      # extra nodes
      for $n (sort { $a <=> $b } keys %nodes_extra) {
        printf FH "I=%d\tt=%.2f\tW=%s\n", $n+$node_id_offset, $nodes_extra{$n}{t}*$framerate, $nodes_extra{$n}{W};
      }
